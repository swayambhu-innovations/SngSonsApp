...
export class ShipmentDetailService {
    ...
    async formatShipment(shipmentData: any) {
        ...
        await(await this.shipmentService.getVendor(shipmentData.vendorData.map((i: any, idx: number) => { return i.vendor }))).docs.map((vendor: any) => {
            ...
            vdata.postalCode && shipmentData.vendorDetails.postalCode.push(vdata.postalCode);
            vdata.distance && shipmentData.vendorDetails.distance.push(vdata.distance);
        });
        ...
        shipmentData.vendorDetails.kot = shipmentData.vendorData.map((item: any) => {
            return item.KOT;
        })

        shipmentData.vendorDetails.GSTNo = shipmentData.vendorDetails.GSTNo.join(', ');
        shipmentData.vendorDetails.WSCode = shipmentData.vendorDetails.WSCode.join(', ');
        shipmentData.vendorDetails.WSName = shipmentData.vendorDetails.WSName.join(', ');
        shipmentData.vendorDetails.WSTown = shipmentData.vendorDetails.WSTown.join(', ');
        shipmentData.vendorDetails.panNo = shipmentData.vendorDetails.panNo.join(', ');
        shipmentData.vendorDetails.phoneNO = shipmentData.vendorDetails.phoneNO.join(', ');
        shipmentData.vendorDetails.postalCode = shipmentData.vendorDetails.postalCode.join(', ');
        shipmentData.vendorDetails.distance = shipmentData.vendorDetails.distance.reduce((acc: number, item: string) => {
            acc += parseInt(item);
            return acc;
          }, 0);
        shipmentData.vendorDetails.kot = shipmentData.vendorDetails.kot.reduce((acc: number, item: string) => {
            acc += parseInt(item);
            return acc;
          }, 0);
