...
export class DaywiseSummaryService {
    ...
    chartData: any = {
        'pending-dispatch': { count: 0, name: 'Pending Dispatch', color: '#DBE5F7' },
        'pending-post-delivery': { count: 0, name: 'Pending Post Delivery', color: '#AACBFF' },
    ...
    async getShipments() {
        ...
        (await this.shipmentsService.getShipmentsByDate(this.selectedDate)).docs.map((shipment: any) => {
